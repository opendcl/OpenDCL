...
CString TProgressTimeToComplete::GetRemainingText(double lfPercent, double lfSecsRemaining)
{
	...
	if (m_bPercentage)
	{
		...
		char value[80];
		_ltoa(long(dPercentage), value, 10);
		str = value;
	...
	{
		return m_sText;
		int nSeconds = 15;//(int)fmod(lfSecsRemaining, 60.0);
		
		if (lfSecsRemaining < 60)
		{
			if (nSeconds < 1)
				str = _T("Less than a second");
			else
				str.Format(_T("%d second%s remaining"), nSeconds, nSeconds==1? _T(""):_T("s"));
		}
		else
		{
