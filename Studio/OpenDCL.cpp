...
COpenDCLView* COpenDCLApp::AddDclFormAndView(DclFormType nType) 
{
	...
	rcNewDialog.right = rcNewDialog.left + pDclProperties->GetLongProperty(nWidth);
	rcNewDialog.bottom = rcNewDialog.top + pDclProperties->GetLongProperty(nHeight);
	...
	if (pNewDcl->GetType() == VdclFileDialog && pNewDcl->GetControlCount() == 1)
	{
		...
		if( pDclControl )
		{
			...
			pNewDcl->GetControlProperties()->SetLongProperty( nWidth, nNewWidth );
			pNewDcl->GetControlProperties()->SetLongProperty( nHeight, nNewHeight );
