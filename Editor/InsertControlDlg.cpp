...
void GetClassServerPath( REFCLSID clsid, CString& strServerPath )
{
   ...
   {
	   CString sText;
	   sText = theWorkspace.LoadResourceString(IDS_CLSID);

	  nResult = RegOpenKeyEx( HKEY_CLASSES_ROOT, sText+CString(szCLSID ), 0, KEY_READ, &hKey );
	  if( nResult != ERROR_SUCCESS )
	  {
		 throw( E_FAIL );
	  }

	   sText = theWorkspace.LoadResourceString(IDS_InprocServer32);
	  nResult = RegOpenKeyEx( hKey, sText, 0, KEY_READ,
		 &hServerKey );
	  if( nResult != ERROR_SUCCESS )
	  {
		  sText = theWorkspace.LoadResourceString(IDS_InprocHandler32);
		 nResult = RegOpenKeyEx( hKey, sText, 0, KEY_READ,
			&hServerKey );
		 if( nResult != ERROR_SUCCESS )
		 {
			 sText = theWorkspace.LoadResourceString(IDS_LocalServer32);
			nResult = RegOpenKeyEx( hKey, sText, 0, KEY_READ,
			   &hServerKey );
			if( nResult != ERROR_SUCCESS )
			{
