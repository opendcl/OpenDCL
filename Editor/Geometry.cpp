...
void CGeometry::ShowSplitter(CString sName) 
{
	...
	if (pCtrl)
	{
		int nTheWidth = pCtrl->GetLngProperty(nWidth);
		int nTheHeight = pCtrl->GetLngProperty(nHeight);
		int nTheLeft = pCtrl->GetLngProperty(nLeft);
		int nTheTop = pCtrl->GetLngProperty(nTop);
		
		if (nTheWidth > nTheHeight)
		{
			if (m_pControl->GetLngProperty(nTop) < nTheTop)
		...
		{
			if (m_pControl->GetLngProperty(nLeft) < nTheLeft)
