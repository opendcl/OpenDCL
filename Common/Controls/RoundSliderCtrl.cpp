...
BOOL CRoundSliderCtrl::Create(DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID)
{
...
void CRoundSliderCtrl::OnPaint() 
{
	...
	pDC->FillRgn(&rgnKnob, CBrush::FromHandle(m_brKnob));
...
CRoundSliderCtrl& CRoundSliderCtrl::SetDialColor(COLORREF crBkgnd)
{
	...
	m_brDial.CreateSolidBrush(crBkgnd);
	return *this;
}
...
CRoundSliderCtrl& CRoundSliderCtrl::SetKnobColor(COLORREF crBkgnd)
{
	...
	m_brKnob.CreateSolidBrush(crBkgnd);
	return *this;
}
...
LRESULT CRoundSliderCtrl::OnSetFont(WPARAM wParam, LPARAM lParam)
{
	CFont::FromHandle( (HFONT)wParam )->GetLogFont( &m_lf );
	m_font.DeleteObject();
	m_font.CreateFontIndirect(&m_lf);
	if( LOWORD(lParam) )
		RedrawWindow();
