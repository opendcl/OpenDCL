...
BOOL CPPToolTip::Create(CWnd* pParentWnd, BOOL bBalloon /* = TRUE */)
{
	...
	DWORD dwExStyle = WS_EX_TOOLWINDOW/* | WS_EX_TOPMOST*/; //WS_EX_TOPMOST causes AutoCAD text screen focus problem [ORW]
...
void CPPToolTip::OnTimer(UINT_PTR nIDEvent)
{
	...
	case TIMER_SHOW:
		...
		else if (PPTOOLTIP_STATE_HIDEN == m_nTooltipState)
		{
			...
			HWND hTargetWnd = ::WindowFromPoint(m_ptOriginal);
			if (hTargetWnd != m_hParentWnd)
			{
				while ((GetWindowLong(hTargetWnd, GWL_STYLE) & WS_CHILD) != 0)
					hTargetWnd = ::GetParent(hTargetWnd);
				if (!::IsChild(hTargetWnd, m_hParentWnd))
