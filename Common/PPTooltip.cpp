...
CPPToolTip::CPPToolTip()
{
	...
	m_BackColor = m_crColor[PPTOOLTIP_COLOR_BK_BEGIN];
...
BOOL CPPToolTip::Create(CWnd* pParentWnd, BOOL bBalloonSize /* = TRUE */) 
{
	...
	// here we are forcing the balloon size to true
	bBalloonSize = true;
...
void CPPToolTip::OnDestroy() 
{
	...
	if (m_pToolInfo.hBitmap != NULL)
		::DeleteObject(m_pToolInfo.hBitmap);
...
void CPPToolTip::Pop()
{
	...
	if (IsWindow(m_Avi.m_hWnd) == TRUE)
	{
		m_Avi.Stop();
		m_Avi.ShowWindow(FALSE);
	}
}
...
void CPPToolTip::OnDraw(CDC * pDC, CRect rect)
{
	...
	if (m_pToolInfo.hIcon != NULL)
	{
		...
		if (IsWindow(m_Avi.m_hWnd) == TRUE || m_AviFileName.GetLength() > 0)
		{
			ptIcon.y = rect.top + 5;
		}
		else
	...
	{
		CPoint ptBitmap;
		ptBitmap.x = m_nSizes[PPTTSZ_MARGIN_CX];
		ptBitmap.y = rect.top;
		...
		{		
			if (m_pToolInfo.nStyles & PPTOOLTIP_ICON_VCENTER_ALIGN)
				ptBitmap.y = rect.top + (rect.Height() - m_szToolIcon.cy) / 2;
			else if (m_pToolInfo.nStyles & PPTOOLTIP_ICON_BOTTOM_ALIGN)
				ptBitmap.y = rect.bottom - m_szToolIcon.cy;
		}
		
		pDC->DrawState(ptBitmap, m_szToolIcon, m_pToolInfo.hBitmap, DSS_NORMAL, NULL);
	...
	CSize sz = PrintTitleString(pDC, rect, m_pToolInfo.sTooltip, FALSE);

	// place the animation control
	if (m_AviFileName.GetLength() > 0)
	{
		if (IsWindow(m_Avi.m_hWnd) == FALSE)
		{
			m_Avi.Create(
				WS_CHILD|WS_CLIPSIBLINGS |
				ACS_CENTER |ACS_AUTOPLAY|ACS_TRANSPARENT, 
				CRect(0,0,m_szAvi.cx,m_szAvi.cy),
				this,
				1010);				
		}	
		if (IsWindow(m_Avi.m_hWnd) == TRUE && m_Avi.IsWindowVisible() == FALSE)
		{
			CRect rcAvi;
			m_Avi.GetClientRect(&rcAvi);

			CRect rcAviNew;

			rcAviNew.top = rect.top + sz.cy+6;
			rcAviNew.left = rect.left;
			rcAviNew.bottom = rcAviNew.top + rcAvi.Height();
			rcAviNew.right = rcAviNew.left + rcAvi.Width();

			m_Avi.MoveWindow(rcAviNew);
			m_Avi.ShowWindow(TRUE);
			m_Avi.Open(m_AviFileName);
		}
	}
...
void CPPToolTip::PrepareDisplayToolTip(CPoint * pt)
{
	...
	if (m_pToolInfo.hBitmap == NULL && m_pToolInfo.hIcon == NULL)
	{
		m_szToolIcon.cx = 0;
		m_szToolIcon.cy = 0;
	}
	//	m_szToolIcon = GetSizeBitmap(m_pToolInfo.hBitmap);
	...
	if (m_AviFileName.GetLength() > 0)
	{
		m_szAvi = ExtractAviSize(m_AviFileName);

		if (m_szAvi.cy == -1)			
			m_AviFileName.Empty();
		else
		{
			rect.bottom += m_szAvi.cy + 5;
			if (rect.Width() < m_szAvi.cx)
			{	
				rect.right = rect.left + m_szAvi.cx + 25;
				if (m_pToolInfo.hIcon != NULL)
					rect.right += m_szToolIcon.cx + 15;
				if (m_pToolInfo.hBitmap != NULL)
					rect.right += m_szToolIcon.cx + 15;		
			}
		}					
...
{
	if (m_pToolInfo.sTooltip.GetLength() <= 34)
		return;
	...
	rgnCombo.CombineRgn(&m_rgnToolTip, 0, RGN_COPY);
	//CopyRgn(rgnCombo.m_hObject, m_rgnToolTip.m_hObject);
	...
	if (m_pToolInfo.nStyles & PPTOOLTIP_SHADOW)
	{
		...
		m_rgnShadow.CombineRgn(&m_rgnToolTip, 0, RGN_COPY);
...
CSize CPPToolTip::GetSizeBitmap(HBITMAP hBitmap) const
{
	CSize sz (0, 0);
	...
	if (hBitmap != NULL)
	{
		// Gets icon dimension
		CSize szBmp;
		::GetBitmapDimensionEx(hBitmap, &szBmp);		
	}
	return sz;
}
///////////////////////////////////////////////////
...
CSize CPPToolTip::PrintTitleString(CDC * pDC, CRect rect, CString str, BOOL bCalculate /* = TRUE */)
{
	...
	int nTheHeight = tm.tmHeight; //The height of the font
	int nTheWidth = tm.tmAveCharWidth; //The width of the font
	...
	int nLineHeight = bCalculate ? nTheHeight : m_nHeightLines.GetAt(0); //The height of the current line
  ...
  int i;	
	for (i = 0; i <= str.GetLength(); i++)
	{
		if (i < str.GetLength())
		{
			...
			else
			{
				...
				if (bCalculate)
					...
					pDC->TextOut(ptCur.x, ptCur.y + m_nHeightLines.GetAt(nLine) - nTheHeight, strText);
				ptCur.x += szLine.cx;
				strText = CString();
				bFirstOutput = FALSE;
				i--;
		...
		case CMD_NEW_LINE:
			...
			bFirstOutput = TRUE;
			ptCur.x = pt.x;
			nLineHeight = nTheHeight;
	//		szLine.cy = nTheHeight;
			break;
		case CMD_TABULATION:
			//Tabulation
			if (!nParam)
				nParam = 1;
			nParam1 = (ptCur.x - pt.x) % (nTheWidth * 4);
			if (nParam1)
			{
				//aligns with tab
				ptCur.x += (nTheWidth * 4) - nParam1;
				nParam --;
			}
			ptCur.x += (nParam * nTheWidth * 4);
			break;
		}
	}
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPToolTip::SetSize(int nSizeIndex, UINT nTheValue)
{
	TRACE(_T("CPPToolTip::SetSize(nSizeIndex = %d, nTheValue = %d)\n"), nSizeIndex, nTheValue);
	if (nSizeIndex >= PPTTSZ_MAX_SIZES)
		return;

	m_nSizes [nSizeIndex] = nTheValue;
}
...
//		nIdBitmap  [in] - the bitmap's identificator 
//
//  Returns :
//		None
//
/////////////////////////////////////////////////////////////////////////////
void CPPToolTip::AddTool(CWnd * pWnd, UINT nIdText, HBITMAP hBitmap /* = NULL */, 
						 LPCRECT lpRectTool /* = NULL */, UINT nIDTool /* = 0 */)
{
	CString str;
    str = theWorkspace.LoadResourceString(nIdText);
	AddTool(pWnd, str, hBitmap, lpRectTool, nIDTool);
} // End AddTool
/////////////////////////////////////////////////////////////////////////////
//  CPPToolTip::AddTool (public member function)
//    adds or updates the tool
...
//		pWnd	[in] - the pointer to the window
//		sTooltipText [in] - the tooltip's text 
//		hBitmap   [in] - the bitmap's handle 
//
//  Returns :
//		None
//
/////////////////////////////////////////////////////////////////////////////
void CPPToolTip::AddTool(CWnd * pWnd, CString sTooltipText, HBITMAP hBitmap /* = NULL */, 
						 LPCRECT lpRectTool /* = NULL */, UINT nIDTool /* = 0 */)
{
	// Store the tool information
	PPTOOLTIP_INFO ti;
	ti.hWnd = pWnd->m_hWnd;//pWnd->GetSafeHwnd();
	ti.nIDTool = nIDTool;
	ti.hBitmap = hBitmap;
	ti.sTooltip = sTooltipText;
	ti.nMask = 0; //All values as default
...
*/
	...
	//	m_arrTools.Add(ti); 
	//else 
	if (m_arrTools.GetSize() > 0)
		m_arrTools.SetAt(0, ti); 
	else
		m_arrTools.Add(ti); 
} // End AddTool
...
void CPPToolTip::UpdateTipText(CString &sText, CWnd *pWnd, int nIndex)
{
	PPTOOLTIP_INFO ti;
	if (!IsExistTool(nIndex-1))
		return;
	ti = m_arrTools.GetAt(nIndex-1);

	ti.sTooltip = sText;
	m_arrTools.SetAt(nIndex-1, ti);
	return;
} // End GetTool
...
//
/////////////////////////////////////////////////////////////////////////////
void CPPToolTip::ShowHelpTooltip(CPoint & pt, CString sTooltipText, HBITMAP hBitmap)
{
	PPTOOLTIP_INFO ti;
	ti.hWnd = m_pParentWnd->GetSafeHwnd();
	ti.hIcon = NULL;
...
void CPPToolTip::ShowHelpTooltip(CPoint & pt, PPTOOLTIP_INFO & ti)
{
	...
	if (m_AviFileName.GetLength() > 0)
	{
		if (IsWindow(m_Avi.m_hWnd) == FALSE)
		{
			CSize szAvi = ExtractAviSize(m_AviFileName);

			if (szAvi.cy != -1)
			{
				m_Avi.Create(
					WS_CHILD|WS_CLIPSIBLINGS |
					ACS_CENTER |ACS_AUTOPLAY, 
					CRect(0,0,szAvi.cx,szAvi.cy),
					this,
					1010);				
			}
			else
	...
	else if (IsWindow(m_Avi.m_hWnd) == TRUE)
	{
		m_Avi.Stop();
		m_Avi.ShowWindow(FALSE);
	}
...
HBRUSH CPPToolTip::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor) 
{
