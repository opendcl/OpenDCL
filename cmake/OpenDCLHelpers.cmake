...
function(opendcl_vs_attach_libdir_props target family release_dirs)
  ...
  # Relative to the .sln (CMAKE_BINARY_DIR); import uses $(SolutionDir).
  file(RELATIVE_PATH _fd_props_rel "${CMAKE_BINARY_DIR}" "${_fd_props_abs}")
  ...
  string(APPEND _xml "  <!-- Generated by OpenDCL CMake: release SDK libdirs (all configs). -->\n")
  string(APPEND _xml "  <!-- Host-debug fulldebug.*.props is imported late (see opendcl_vs_disable_manifest). -->\n")
  ...
  string(APPEND _xml "  <Import Project=\"$(SolutionDir)${_local_props_rel}\"\n")
  string(APPEND _xml "         Condition=\"'$(SolutionDir)' != '' and exists('$(SolutionDir)${_local_props_rel}')\" />\n")

  # Same shape for every config. When fulldebug.*.props is imported later, it
  # prepends host paths via host;%(AdditionalLibraryDirectories).
  if(_rel_ms)
  ...
  # Stash SolutionDir-relative fulldebug path for late import (after vcxproj
  # LocalDebuggerCommand). Empty if the machine sheet is absent.
  if(EXISTS "${_fd_props_abs}")
    set_property(TARGET ${target} PROPERTY OPENDCL_FULLDEBUG_PROPS_REL
      "${_fd_props_rel_ms}")
  else()
    set_property(TARGET ${target} PROPERTY OPENDCL_FULLDEBUG_PROPS_REL "")
  endif()

  # FullDebug /MDd is set on the target via MSVC_RUNTIME_LIBRARY when the machine
  # fulldebug.<family>.props exists (props RuntimeLibrary cannot override a later
...
function(opendcl_vs_disable_manifest target)
  ...
  get_property(_fd_rel TARGET ${target} PROPERTY OPENDCL_FULLDEBUG_PROPS_REL)
  set(_xml "")
  string(APPEND _xml "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n")
  string(APPEND _xml "<Project xmlns=\"http://schemas.microsoft.com/developer/msbuild/2003\">\n")
  string(APPEND _xml "  <!-- OpenDCL: classic runtime modules do not embed manifests (no mt.exe). -->\n")
  # Import machine fulldebug.*.props *after* vcxproj LocalDebuggerCommand so the
  # sheet owns FullDebug host Command / includes / libs (no paths in-repo).
  if(_fd_rel)
    string(APPEND _xml "  <Import Project=\"$(SolutionDir)${_fd_rel}\"\n")
    string(APPEND _xml "         Condition=\"'$(Configuration)'=='FullDebug' and '$(SolutionDir)' != '' and exists('$(SolutionDir)${_fd_rel}')\" />\n")
  endif()
