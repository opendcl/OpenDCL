...
static const int iHeightAdjustment = 27; //should be calculated from system metrics [ORW]
static const int iWidthAdjustment = 8; //should be calculated from system metrics [ORW]
...
static const struct AdsFunctionTableEntry { LPCTSTR pszFunctionName; int (*pfHandler) (); } grAdsFunctionTable[] =
{
	...
	{sAnimate_Load,			AnimateCtrl_Load},
	{sAnimate_Seek,			AnimateCtrl_Seek},
	{sAnimate_Stop,			AnimateCtrl_Stop},
	{sAnimate_Close,		AnimateCtrl_Close},

	{sHatch_Clear,			Hatch_Clear},
	{sHatch_SetPattern,		Hatch_SetPattern},
...
public:
	...
	static int ads_odcl_new_dialog(void)
	{
		...
		//if (pProjectToBeShown->m_PurchaseState == 96)
		//{
		//	MessageBox(
		//		FindTopModalForm(), 
		...
		//}
		//else if (pProjectToBeShown->m_PurchaseState != nPurchasedMode &&
		//	pProjectToBeShown->m_PurchaseState != 400 &&
		//	pProjectToBeShown->m_PurchaseState != 401 &&
		//	pProjectToBeShown->m_PurchaseState != 402 &&
		//	pProjectToBeShown->m_PurchaseState != 403)
		//{
	...
	static int ads_odcl_activateemail(void)
	{
		...
		if (pArgs)
		{
			if (pArgs->restype != RTSTR)
				return RSERR; //wrong argument type
			pszSubject = pArgs->resval.rstring;
