...
void CButtonST::FreeResources(BOOL bCheckForNULL)
{
	...
	if (m_pPressedPicture != NULL && m_bLoadPicture)
	{
		delete m_pPressedPicture;
		m_pPressedPicture = NULL;
	}

	if (bCheckForNULL && m_bLoadPicture)
	{
		// Destroy icons
...
void CButtonST::FreePressedResources(BOOL bCheckForNULL)
{
	if (m_pPressedPicture != NULL && m_bLoadPicture)
	{
		delete m_pPressedPicture;
...
void CButtonST::FreeUnPressedResources(BOOL bCheckForNULL)
{
	if (m_pPicture != NULL && m_bLoadPicture)
	{
		//delete m_pPicture;
...
void CButtonST::PreSubclassWindow() 
{
	...
	m_nTypeStyle = nBS & ODCL_TYPEMASK;
	...
	ModifyStyle(ODCL_TYPEMASK, BS_OWNERDRAW, SWP_FRAMECHANGED);
...
{
	UINT nNewType = (wParam & ODCL_TYPEMASK);
	...
	return DefWindowProc(BM_SETSTYLE,
		(wParam & ~ODCL_TYPEMASK) | BS_OWNERDRAW, lParam);
} // End of OnSetStyle
...
void CButtonST::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (wndUnderMouse && wndUnderMouse->m_hWnd == m_hWnd && wndActive)
	{
		if (!m_bMouseOnButton)
		{
			...
			if (m_ArxControl->GetLngProperty(nButtonStyle) != 5)
			{
				Invalidate();
...
void CButtonST::OnDoubleclicked() 
{
	if (m_ArxControl)
	{
...
{	
	...
	if (m_sOnClickedEvent.GetLength() > 0)
	{
		CSpreadSheet *pParent = (CSpreadSheet*)GetParent();
		// call methods to invoke the event
		InvokeMethodIntInt(m_sOnClickedEvent, pParent->m_nRowSelected, pParent->m_nColSelected, m_bInvokeWithSendString);
	}
...
//#endif
	...
	{	
			...
			{
					if (m_bIsFlat == TRUE && !m_bMouseOnButton)
					{
						// draw the solid rectangle
						::SetBkColor(pDC->m_hDC, ::GetSysColor(COLOR_BTNFACE));
						::ExtTextOut(pDC->m_hDC, 0, 0, ETO_OPAQUE, &itemRect, NULL, 0, NULL);
					}
					else
	...
	if (m_BackColor != ::GetSysColor(COLOR_BTNFACE))
	{
		CBrush br(m_BackColor);
		CRect backRect = itemRect;
		if (!m_bHasBorder)
			backRect.DeflateRect(5, 4);			
	...
	if (m_pPicture)
	{
		...
		int nPicWidth = IconSize.cx;
		int nPicHeight = IconSize.cy;
		CRect rcCell;
		...
		{			
			if (m_pPicture->GetBitmap() == NULL)
			{
				HBITMAP hBitmap;
				m_pPicture->GetPicture().m_pPict->get_Handle((OLE_HANDLE FAR *) &hBitmap);
				m_pPicture->SetBitmap(hBitmap);
			}
		...
		{
			HICON hIconOut = m_pPicture->GetIcon();
			
			if (nEnabled)
			{
				DrawState(pDC->m_hDC, NULL, NULL,
						(LPARAM)hIconOut, 0, rcPic.left, rcPic.top, IconSize.cx, IconSize.cy, DSS_NORMAL|DST_ICON);
			}
			else
			{				
				DrawState(pDC->m_hDC, NULL, NULL,
						(LPARAM)hIconOut, 0, rcPic.left, rcPic.top, IconSize.cx, IconSize.cy, DSS_DISABLED|DST_ICON);
			}		
		}
	}
	...
	// Draw the icon
	else if (m_csIcons[0].hIcon)
	{
		DrawTheIcon(pDC, !sTitle.IsEmpty(), &rcImage, &captionRect, m_bIsPressed, m_bIsDisabled);
	} // if

	else if (m_csBitmaps[0].hBitmap)
	{
		pDC->SetBkColor(m_BackColor);
		DrawTheBitmap(pDC, !sTitle.IsEmpty(), &rcImage, &captionRect, m_bIsPressed, m_bIsDisabled);
	} // if

	// Write the button title (if any)
	if (sTitle.IsEmpty() == FALSE)
	{
		DrawTheText(pDC, (LPCTSTR)sTitle, &rcImage, &captionRect, m_bIsPressed, m_bIsDisabled);
	} // if

	if (m_bIsFlat == FALSE || (m_bIsFlat && m_bDrawFlatFocus))
	{
		// Draw the focus rect
		if (m_bIsFocused && m_ArxControl)
		{
...
HBITMAP CButtonST::CreateBitmapMask(HBITMAP hSourceBitmap, DWORD dwWidth, DWORD dwHeight, COLORREF crTransColor)
{
	...
	crSaveDestText = ::SetTextColor(hdcSrc, m_ForeColor);
...
void CButtonST::SetAcadColor(long nColor)
{
...
void CButtonST::SetForeColor(long nColor)
{
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (bIsDisabled && m_bShowDisabledBitmap)
	{
		...
		COLORREF crOldColor = ::SetBkColor(hDC, m_BackColor);
...
void CButtonST::SetDragnDrop(BOOL bRegister)
{
	if (bRegister == TRUE)
	{
...
void CButtonST::SetPictureID(int nPictureID)
{
	...
	int nCount = m_pProject->GetPictureList().GetCount();
	...
	while (pos != NULL)
	{
		...
		if (pPicture->GetID() == nPictureID)
		{
			m_pPicture = pPicture;
/*			
			long lValue = m_ArxControl->GetLngProperty(nPressedPicture);
				
			int n = m_pPicture->m_hPicture.GetType();
			if (m_pPicture->m_hPicture.GetType() == PICTYPE_ICON && m_pPicture->m_hIcon != NULL)				
			{
				if (lValue > 0)
					SetIcon(NULL, ::CopyIcon(m_pPicture->m_hIcon));
				else
					SetIcon(::CopyIcon(m_pPicture->m_hIcon), NULL);
				return;
			}
			else if (m_pPicture->m_hPicture.GetType() == PICTYPE_ICON && m_pPicture->m_hIcon == NULL)
			{
				m_pPicture->m_hPicture.m_pPict->get_Handle((OLE_HANDLE FAR *) &m_pPicture->m_hIcon);
				if (lValue > 0)
					SetIcon(NULL, ::CopyIcon(m_pPicture->m_hIcon));
				else
					SetIcon(::CopyIcon(m_pPicture->m_hIcon), NULL);
			...
			else if (m_pPicture->m_PicType == PICTYPE_ICON)
			{
				if (lValue > 0)
					SetIcon(NULL, ::CopyIcon(m_pPicture->m_hIcon));
				else
...
void CButtonST::SetPressedPictureID(int nPictureID)
{
	if (nPictureID == 0 && m_pPressedPicture != NULL && m_bLoadPicture)
	{
		m_pPressedPicture = NULL;
		delete m_pPressedPicture;
		SetIcon(NULL, NULL);
	...
	while (nCount < m_pProject->GetPictureList().GetCount())
	{
		...
		if (pPicture->GetID() == nPictureID)
		{
			if (CString(pPicture->GetFileName()).GetLength() > 0)
			{
				...
				m_pPressedPicture = new CPictureObject;
				
				m_pPressedPicture->SetID(pPicture->GetID());
				m_pPressedPicture->SetFileName(pPicture->GetFileName());

				m_pPressedPicture->EnsurePictureIsLoaded();
			}
			else
			{
				m_pPressedPicture = pPicture;
			}

			if (m_pPressedPicture->GetPicType() == PICTYPE_ICON && m_pPressedPicture->GetIcon() != NULL)				
			{
				SetIcon(::CopyIcon(m_pPressedPicture->GetIcon()), NULL);
				return;
			}
			else if (m_pPressedPicture->GetPicType() == PICTYPE_ICON)
			{
				SetIcon(::CopyIcon(m_pPressedPicture->GetIcon()), NULL);
				return;
			}			
			
			else if (m_pPressedPicture->GetPicType() == PICTYPE_BITMAP && m_pPressedPicture->GetBitmap() != NULL)
			{
				HICON icon = m_pPressedPicture->GetIcon();
				SetIcon(icon, NULL);
				return;
			}

			else if (m_pPressedPicture->GetPicType() == PICTYPE_BITMAP)
			{
				//m_pPressedPicture->m_hPicture.m_pPict->get_Handle((OLE_HANDLE FAR *) &m_pPressedPicture->m_hBitmap);
				//SetBitmaps(
				HICON icon = m_pPressedPicture->GetIcon();
				
				//SetIcon(::CopyIcon(m_pPressedPicture->m_hIcon), m_csIcons[1].hIcon);
				SetIcon(icon, NULL);
				return;
			}
			else if (m_pPressedPicture->GetPicType() == PICTYPE_ICON)
