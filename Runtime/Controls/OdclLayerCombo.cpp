...
OdclLayerCombo::OdclLayerCombo()
{
	...
	hIcon = (HICON)::LoadImage(hRes, MAKEINTRESOURCE(IDI_LAYERTHAWED), IMAGE_ICON, 0, 0, 0);	
	m_Images.Add(hIcon);
	DestroyIcon(hIcon);

	hIcon = (HICON)::LoadImage(hRes, MAKEINTRESOURCE(IDI_LAYERFROZEN), IMAGE_ICON, 0, 0, 0);
	m_Images.Add(hIcon);
	DestroyIcon(hIcon);
