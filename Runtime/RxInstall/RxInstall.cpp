...
public:
	...
	enum Target
	{
		...
		kGstarCAD2027x64 =      (kX64Architecture | kGstarCADPlatform | (kGRX2027 << shftMajorVer) | 0),
	};
			...
			case kGstarCAD:
				{
				  MajorVersion ver = majorVersion();
					if (ver == kZRX2024)
						ver = kZRX2024; // ZWCAD 2024 loads ZRX 2023 binary
					String sVersion( 2000 + ver );
					sAppFilename += sVersion;
					sAppFilename += _T(".grx");
				}
				break;
...
void InstallAllTargets( LPCTSTR pszInstallDir, bool bWantHKLM, bool bLoadOnStartup )
{
	...
	EnumerateRegTargets( TargetModule( TargetModule::kGstarCAD2022x86, pszInstallDir ), bWantHKLM, bLoadOnStartup );
	...
	EnumerateRegTargets( TargetModule( TargetModule::kZWCAD2023x86, pszInstallDir ), bWantHKLM, bLoadOnStartup );
	EnumerateRegTargets( TargetModule( TargetModule::kZWCAD2024x86, pszInstallDir ), bWantHKLM, bLoadOnStartup );
	...
	if( IsWow64() )
	{
		...
		EnumerateRegTargets( TargetModule( TargetModule::kGstarCAD2027x64, pszInstallDir ), bWantHKLM, bLoadOnStartup );
...
void UninstallAllTargets( HKEY hkRoot )
{
	...
	RemoveAllRegTargets( _T("ZWSOFT\\ZWCAD\\2023"), hkRoot );
	RemoveAllRegTargets( _T("ZWSOFT\\ZWCAD\\2024"), hkRoot );
	...
	if( IsWow64() )
	{
		...
		RemoveAllRegTargets( _T("Gstarsoft\\GstarCAD\\R27"), hkRoot, true );
	}
}
